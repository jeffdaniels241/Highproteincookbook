...
import { proteinTypeLabels, type ProteinType } from '../src/data/proteinProfiles';
...
export default function Home() {
  ...
  const [taste, setTaste] = useState<'all' | 'sweet' | 'savory' | 'dessert'>('all');
  const [proteinType, setProteinType] = useState<'all' | ProteinType>('all');
  ...
  const filtered = useMemo(() => {
    ...
    const result = visibleRecipes.filter((recipe) => {
      ...
      const matchesTaste = taste === 'all' || (taste === 'dessert' ? recipe.isDessert : recipe.taste === taste);
      const matchesProteinType = proteinType === 'all' || recipe.proteinType === proteinType;
      ...
      return matchesQuery && matchesCategory && matchesProtein && matchesVegetarian && matchesTaste && matchesProteinType && matchesPantry;
    });
  ...
  }, [category, favorites, pantry, pantryMode, proteinOnly, proteinType, query, sort, taste, vegetarianOnly, visibleRecipes]);
