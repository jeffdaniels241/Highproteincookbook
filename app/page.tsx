...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    const result = recipes.filter((recipe) => {
      ...
      const matchesProtein = !proteinOnly || recipe.tags.includes('high-protein') || recipe.nutrition.protein >= 25;
  ...
  }, [category, favorites, pantry, pantryMode, proteinOnly, query, sort, vegetarianOnly]);
  ...
  return <main className="site-shell">
    <header className="topbar"><Link className="brand" href="/"><span className="brand-mark">HPC</span><span>High-Protein<br /><strong>Rice Cooker Cookbook</strong></span></Link><nav><Link href="/rice-cooker-guide">Rice cooker guide</Link><a href="#recipes">Browse recipes</a></nav></header>
    <section className="hero"><div><p className="eyebrow">127 practical recipes · local-first library</p><h1>Cook protein-rich meals<br /><em>without the guesswork.</em></h1><p className="hero-copy">A candid recipe library for rice cookers, ovens, and skillets—organized around simple ingredients, honest testing status, and meals worth making again.</p></div><div className="hero-card"><span className="hero-card-label">Today’s shortcut</span><strong>{recipes.filter((r) => r.riceCooker).length} rice-cooker recipes</strong><span>Use the guide before your first batch.</span><Link href="/rice-cooker-guide">Learn the method →</Link></div></section>
    <section className="controls" aria-label="Recipe filters"><div className="search-wrap"><label htmlFor="recipe-search">Search recipes</label><input id="recipe-search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Try chicken, oats, lemon…" /></div><button className="filter-toggle" onClick={() => setShowPantry((v) => !v)} aria-expanded={showPantry}>Pantry {pantry.length ? `(${pantry.length})` : ''}</button><label className="select-label">Sort<select value={sort} onChange={(e) => setSort(e.target.value)}><option value="recommended">Recommended</option><option value="protein">Highest protein</option><option value="calories">Lowest calories</option><option value="time">Fastest</option><option value="favorites">Favorites first</option><option value="alpha">A–Z</option></select></label><label className="check-label"><input type="checkbox" checked={proteinOnly} onChange={(e) => setProteinOnly(e.target.checked)} /> Protein+</label><label className="check-label"><input type="checkbox" checked={vegetarianOnly} onChange={(e) => setVegetarianOnly(e.target.checked)} /> Vegetarian</label></section>
    <div className={`library-layout ${showPantry ? 'pantry-open' : ''}`}><aside className="sidebar" aria-label="Categories and pantry"><div className="sidebar-heading"><span>Browse</span><button className="close-mobile" onClick={() => setShowPantry(false)} aria-label="Close pantry filters">×</button></div><button className={category === 'all' ? 'category active' : 'category'} onClick={() => setCategory('all')}><span>All recipes</span><b>{recipes.length}</b></button>{categoryOrder.map((key) => <button key={key} className={category === key ? 'category active' : 'category'} onClick={() => setCategory(key)}><span>{categoryLabels[key]}</span><b>{recipes.filter((recipe) => recipe.category === key).length}</b></button>)}<div className="sidebar-section"><div className="sidebar-heading"><span>Pantry match</span><span className="muted">{pantry.length} selected</span></div><div className="mode-switch"><button className={pantryMode === 'all' ? 'active' : ''} onClick={() => setPantryMode('all')}>All</button><button className={pantryMode === 'any' ? 'active' : ''} onClick={() => setPantryMode('any')}>Any</button></div>{pantryOptions.map((item) => <label className="pantry-item" key={item}><input type="checkbox" checked={pantry.includes(item)} onChange={() => togglePantry(item)} />{item}</label>)}</div></aside><section className="results" id="recipes"><div className="results-heading"><div><p className="eyebrow">Your library</p><h2>{filtered.length} recipes</h2></div><span className="status-key"><i className="dot adapted" /> adapted <i className="dot concept" /> needs a cook test</span></div><div className="recipe-grid">{filtered.map((recipe) => <RecipeCard key={recipe.id} recipe={recipe} favorite={favorites.includes(recipe.id)} onFavorite={() => toggleFavorite(recipe.id)} />)}</div>{!filtered.length && <div className="empty"><h3>Nothing matched those filters.</h3><p>Try removing a pantry item or searching a broader ingredient.</p></div>}</section></div>
  </main>;
}

function RecipeCard({ recipe, favorite, onFavorite }: { recipe: Recipe; favorite: boolean; onFavorite: () => void }) {
  return <article className="recipe-card"><div className="card-top"><span className="recipe-category">{categoryLabels[recipe.category]}</span><button className="favorite" onClick={onFavorite} aria-label={`${favorite ? 'Remove' : 'Add'} ${recipe.title} ${favorite ? 'from' : 'to'} favorites`} aria-pressed={favorite}>{favorite ? '♥' : '♡'}</button></div><Link href={`/recipes/${recipe.slug}`} className="card-link"><h3>{recipe.title}</h3><p>{recipe.description}</p><div className="metric-row"><span><strong>{recipe.nutrition.protein}g</strong> protein</span><span><strong>{recipe.nutrition.calories}</strong> cal</span><span><strong>{recipe.totalMinutes}m</strong> total</span></div><div className="card-footer"><span>{recipe.yield.amount} {recipe.yield.label}</span><span className={`recipe-status ${recipe.status}`}>{recipe.status === 'concept' ? 'Concept testing' : 'Adapted'}</span></div></Link></article>;
}
